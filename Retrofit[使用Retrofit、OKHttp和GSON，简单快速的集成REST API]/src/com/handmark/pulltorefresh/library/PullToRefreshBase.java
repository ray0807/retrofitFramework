...
//            default:
    ...
    // add by zhaocl, 2015-05-06, for auto refreshing, begin
    /**
     * 
     * @author 赵成龙
     * @Description 自动刷新，必须实现两个监听(OnAutoRefreshListener、OnRefreshListener2)
     * @return void
     * @date 2015年5月6日 下午3:39:01
     */
    public void autoRefreshing() {
    	if (mHeaderLayout == null || mState != State.RESET 
    			|| mCurrentMode != Mode.PULL_FROM_START) {
			return;
		}
    	...
    	Animation animation = new Animation() {
			...
			protected void applyTransformation(float interpolatedTime,
					Transformation t) {
				final int itemDimension = getHeaderSize();
				final int newScrollValue = (int)(itemDimension * interpolatedTime);
		        ...
		        if (newScrollValue != 0 && !isRefreshing()) {
		            ...
		            if (mState != State.REFRESHING && itemDimension == Math.abs(newScrollValue)) {
		            	if (null == mOnRefreshListener && null == mOnRefreshListener2) {
		            		throw new IllegalStateException("mOnRefreshListener or mOnRefreshListener2 must not be null.");
		            	} 
		            	
		            	if (autoRefreshListener == null) {
		            		throw new IllegalStateException("autoRefreshListener must not be null.");
		            	}
		            	
		            	setState(State.REFRESHING, true);
		            	autoRefreshListener.onAutoRefreshing();
		            }
		        }
			}
		...
		mHeaderLayout.startAnimation(animation);
    }
    ...
    public OnAutoRefreshListener autoRefreshListener;
    
    public void setAutoRefreshListener(OnAutoRefreshListener listener) {
    	autoRefreshListener = listener;
    }
    // add by zhaocl, 2015-05-06, for auto refreshing, end
